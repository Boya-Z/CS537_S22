...
userinit(void)
{
  ...
  // writes to be visible, and the lock is also neededs
...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != curproc)
        continue;
      // check if it is a thread not child process
      flag = 0;
    	for(p_t = ptable.proc; p_t < &ptable.proc[NPROC]; p_t++){
    		if(p_t->pgdir == p->pgdir && p_t->pid != p->pid) {
        	flag = 1;
        	break;
    		}
    	}
