...
thread_join()
{
  void *stack;
  int childPid;
  //Need to calls join() to free user stack and return waited-for PID
  childPid = join(&stack);
  if (childPid != -1) {
    for(int i=0; i<64; i++) {
...
//   int myturn = fetch_add(&lock->ticket, 1);
...
lock_acquire(lock_t *lock){	
  int myturn = fetch_add(&lock->ticket, 1);	
  while(fetch_add(&lock->turn, 0)!= myturn);	
}

void lock_release(lock_t *lock) {
  fetch_add(&lock->turn, 1);
}